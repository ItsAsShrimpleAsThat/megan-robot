...
void loop()
{
  ...
  uint64_t      data = analogRead(leftX)           | ((uint64_t)analogRead(leftY) << 10)    | (uint64_t)(analogRead(rightX) << 20);   //| ((uint64_t)analogRead(rightY) << 30)
...
//                    | (digitalRead(button5) << 44) | (digitalRead(button6) << 45) | (digitalRead(button7) << 46) | (digitalRead(button8) << 47);
  ...
  uint64_t grink = (data >> 10) & 0b1111111111;
